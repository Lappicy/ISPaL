...
Esta pasta deve permanecer dentro do repositório principal, nesta posição:

```text
ISPaL/
├── R/                    # código do pacote ISPaL original
├── data/
├── man/
...
Depois, no R ou RStudio:

```r
setwd("/caminho/para/ISPaL")
source("ISPaL_Shiny/install_dependencies.R") # necessário apenas uma vez
source("ISPaL_Shiny/run_app.R")
runISPaL()
```

Nas utilizações seguintes, basta executar os dois últimos comandos. Também é
possível iniciar a aplicação de qualquer diretório indicando o caminho local
completo do arquivo:

```r
source("/caminho/para/ISPaL/ISPaL_Shiny/run_app.R")
runISPaL()
```
...
Adicione a pasta inteira `ISPaL_Shiny` dentro da raiz do repositório
[`Lappicy/ISPaL`](https://github.com/Lappicy/ISPaL). A estrutura que deve ser
enviada é:
