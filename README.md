...
Pacote R para previsões periódicas com os modelos PAR, PARX e RIDGE e
variáveis exógenas. O pacote inclui as funções originais do ISPaL e a
interface Shiny iniciada por `runISPaL()`.

O nome local da pasta pode ser `ISPaL_Shiny`, mas o nome do pacote definido no
`DESCRIPTION` é `ISPaL`. Todo o conteúdo desta pasta deve ser enviado para a
raiz do repositório `Lappicy/ISPaL`.
...
Depois de enviar o conteúdo desta pasta para a raiz do repositório
[`Lappicy/ISPaL`](https://github.com/Lappicy/ISPaL), instale e execute no R:
...
`lubridate`, `MASS`, `forecast`, `pls`, `glmnet`, `jsonlite`, `rjson`,
`foreign` e `testthat`.

## Como publicar no GitHub

No computador, abra a pasta local `ISPaL_Shiny` e envie **todo o conteúdo
dela** para a raiz do repositório [`Lappicy/ISPaL`](https://github.com/Lappicy/ISPaL).
A estrutura no GitHub deve ser:
...
No GitHub não deve existir uma pasta externa chamada `ISPaL_Shiny`: esse é
apenas o nome da pasta de preparação no computador. O arquivo `DESCRIPTION`
deve aparecer diretamente na página inicial do repositório.
...
Esses comandos locais são opcionais; depois da instalação pelo GitHub, use
`ISPaL::runISPaL()`.
