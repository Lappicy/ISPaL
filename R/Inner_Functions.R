...
select.pred <-
  function(lag, which.month, dates, y.reg, x.reg, ...){
    ...
    yreg <- stats::window(y.reg, deltat = 1,
                          start = c(dates[1] + lag.year, which.month),
                          end = c(dates[2], which.month))
    xreg <- stats::window(x.reg, deltat = 1,
...
select.lambda <-
  function(formula, data, lambda = seq(0.5, 50, by = 0.05), ...){
    ...
    ridge <- MASS::lm.ridge(formula, data, lambda = lambda)
...
cv1 <-
  function(DATA.1, DATA.2, formula, model.name = ""){
    ...
    reg <- stats::lm(formula, DATA.1)

    # Fazer a previsão na validação com intervalos de confiança e prediction
    L <- stats::predict(reg, DATA.2)
    ci.conf <- stats::predict(reg, DATA.2, interval = c("confidence"))[,2:3]
    ci.pred <- stats::predict(reg, DATA.2, interval = c("prediction"))[,2:3]
    ...
    CV[[5]] <- stats::AIC(reg, k = length(reg$coefficients))
  ...
  function(DATA.1, DATA.2, formula, model.name = ""){
    ...
    lambda <- select.lambda(formula, data = DATA.1,
                            lambda = c(seq(0, 99.95, by = 0.05),
                                       seq(100, 1000, by = 1)))
    aux <- MASS::lm.ridge(formula, DATA.1, lambda = lambda)
    beta <- coef(aux)
    ...
    r2.fit <- (stats::cor(L.c, DATA.1$y, use = "complete.obs"))^2 # R2-coef!!
